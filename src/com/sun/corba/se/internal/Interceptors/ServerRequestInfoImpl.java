/*
 * @(#)ServerRequestInfoImpl.java	1.37 03/01/23
 *
    ...
    implements ServerRequestInfo 
{
	...
	isDynamic = false;
	poaimpl = null;
	serverRequestId = piOrb.allocateServerRequestId();
     ...
     */
    ServerRequestInfoImpl( PIORB piOrb ) { 
        super( piOrb ); 
        startingPointCall = CALL_RECEIVE_REQUEST_SERVICE_CONTEXT;
        intermediatePointCall = CALL_RECEIVE_REQUEST;
        endingPointCall = CALL_SEND_REPLY;
	...
	if( cachedSendingException == null ) {
		...
		throw new INTERNAL(
		    "Exception not available in sending_exception.",
		    MinorCodes.EXCEPTION_UNAVAILABLE, 
	...
	if( objectId == null ) {
	    ...
	    throw new NO_RESOURCES( "object id never set.", 
	...
	if( adapterId == null ) {
	    // For some reason, we never set the adapter id.  This could be 
	    // because a servant locator caused a location forward or
	    // raised an exception.  As per ptc/00-08-06, section 21.3.14,
	    // we throw NO_RESOURCES
	    throw new NO_RESOURCES( "adapter id never set.", 
		MinorCodes.PI_OPERATION_NOT_SUPPORTED, 
		CompletionStatus.COMPLETED_NO );
	}
	...
	if( poaimpl != null ) {
	    result = poaimpl.get_effective_policy( type );
	}
	...
	else if( servant instanceof ObjectImpl ) {
	    result = ((ObjectImpl)servant)._is_a( id );
	}
	else {
	    throw new INTERNAL( 
		"During target_is_a, servant was not a Servant or ObjectImpl",
		MinorCodes.SERVANT_INVALID,
		CompletionStatus.COMPLETED_NO );
	}
	...
	if( cachedArguments == null ) {
		...
		throw new NO_RESOURCES(
		    "ServerRequest::arguments() never called.",
		    MinorCodes.PI_OPERATION_NOT_SUPPORTED,
		    CompletionStatus.COMPLETED_NO );
	    }
	...
	throw new NO_RESOURCES(
	    "The Portable Java Bindings do not support operation_context()",
	    MinorCodes.PI_OPERATION_NOT_SUPPORTED,
	    CompletionStatus.COMPLETED_NO );
    }
	...
	if( dsiResult == null ) {
	    throw new INTERNAL(
		"dsiResult should not be null.",
		MinorCodes.PI_DSI_RESULT_IS_NULL,
		CompletionStatus.COMPLETED_NO );
	}
     ...
     */
    protected void setInfo( ServerRequest request, POAImpl poaimpl, 
	byte[] objectId, byte[] adapterId ) 
    {
	...
	this.connection = 
	    ((com.sun.corba.se.internal.iiop.IIOPInputStream)request)
	    .getConnection();
    }
