 ...
 *
 * <p>
 * <em>"Key typed" events</em> are higher-level and do not depend on the platform
 * or keyboard layout. They are generated when a character is entered, and are the
 * preferred way to find out about character input.
 * In the simplest case, a key typed event is produced by a combination of a
 * key press followed by a key release. Often, however, characters are
 * produced by series of key presses and key releases, and the mapping
 * from key pressed/released events to key typed events may be n:1 or n:m.
 ...
 * will be fired with VK_A. Seperately, a KEY_TYPED event with a keyChar 
 * value of 'A' is generated.
 *
 * <P>
 * Notes:
 * <ul>
 * <li>Key combinations which do not result in characters,
...
public class KeyEvent extends InputEvent {
     ...
     * entered. In the simplest case, it is produced by a combination of a
     * key press followed by a key release. Often, however, characters are
     * produced by series of key presses and key releases, and the mapping
     * from key pressed/released events to key typed events may be n:1 or n:m.
     */
